...
public class AttendanceRecordDao {
	...
	public List<AttendanceRecord> getAttendanceForStudentAndSubject(Long studentId, Long subjectId) {

		Session session = null;
		List<AttendanceRecord> list = null;
		try {
			session = factory.openSession();
			Criteria criteria = session.createCriteria(AttendanceRecord.class, "attendance");

			// Join with Subject
			criteria.createAlias("attendance.subject", "subject");

			// Join with Students
			criteria.createAlias("attendance.students", "student");
			...
			criteria.addOrder(Order.asc("attendance.date"));
